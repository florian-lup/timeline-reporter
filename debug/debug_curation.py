...
def create_sample_leads() -> list[Lead]:
    ...
    sample_texts = [
        (
            "President Trump announced frameworks for new trade agreements with Japan and "
            "the Philippines, reducing U.S. tariffs on Japanese imports to 15% and Philippine "
            "goods to 19%. The deals include enhanced military cooperation, particularly "
            "significant amid China's aggression in the South China Sea, with Philippine "
            "President Marcos stating the U.S. remains their 'strongest partner.' This "
            "strengthens U.S. economic and strategic positioning in the Indo-Pacific."
        ),
        (
            "Trump called for criminal charges against former President Barack Obama and "
            "other officials, alleging treason over altered intelligence reports related to "
            "Trump-Russia investigations. This follows a criminal referral by Director of "
            "National Intelligence Tulsi Gabbard, who cited a '180-degree shift' in "
            "intelligence assessments during the 2016–2017 transition period, escalating "
            "political tensions in Washington."
        ),
        (
            "Humanitarian catastrophe deepens in Gaza as UN Secretary-General António "
            "Guterres warns of 'starvation knocking on every door.' Cease-fire negotiations "
            "are deadlocked over food aid distribution, with EU foreign policy chief Kaja "
            "Kallas threatening 'all options remain on the table' if Israel fails to improve "
            "access. U.S. envoy Steve Witkoff is mediating talks in Rome and Qatar to "
            "finalize a hostage deal."
        ),
        (
            "Turkey vowed direct intervention to prevent Syria's fragmentation after "
            "clashes in the south, with Foreign Minister Hakan Fidan pledging to block "
            "militant autonomy. This aligns with Turkey's increasing regional assertiveness, "
            "as President Erdoğan praised the nation's defense industry growth at the IDEF "
            "expo, where localization surpassed 80%."
        ),
        (
            "Ukraine saw its largest wartime protests targeting President Zelensky after he "
            "signed legislation weakening anti-corruption agencies. Protesters condemned what "
            "they called concessions to 'Russian influence,' while Zelensky defended the move "
            "as necessary for sovereignty. Meanwhile, Ukraine's military commander urgently "
            "requested more U.S. and European air defenses against intensified Russian strikes."
        ),
        (
            "The U.S. imposed sanctions on a Houthi-linked petroleum smuggling network "
            "spanning Yemen and the UAE, targeting Iran-backed militant activities. This "
            "coincides with fresh sanctions against the Houthis as part of broader efforts "
            "to disrupt Iran's regional influence and curtail its destabilizing proxy "
            "warfare in the Middle East."
        ),
        (
            "China rebuked the U.S. for withdrawing from UNESCO—its third exit—calling it "
            "'not what a major country should do.' This occurred amid U.S.-China trade talks "
            "scheduled in Stockholm, where Treasury Secretary Scott Bessent will discuss "
            "extending tariff truces and China's oil purchases from Russia/Iran. Trump also "
            "teased a 'not too distant' visit to China."
        ),
        (
            "Portugal endorsed Morocco's Western Sahara autonomy plan, with Foreign Minister "
            "Paulo Rangel calling it a 'serious and credible basis.' This aligns with growing "
            "Western support for Morocco's proposal, contrasting Spain's previous stance and "
            "shifting diplomatic dynamics in North Africa amid regional instability."
        ),
        (
            "The International Court of Justice will issue a landmark advisory opinion on "
            "state obligations to combat climate change and legal consequences for "
            "emissions-causing harm. While non-binding, the ruling is expected to influence "
            "global climate litigation and pressure nations to strengthen environmental "
            "policies under international law."
        ),
        (
            "A Syrian American was executed during sectarian violence in Sweida, Syria, "
            "highlighting escalating tensions. The killing follows anti-government protests "
            "in the Druze-majority region, underscoring Syria's persistent instability and "
            "the risks facing diaspora returnees amid the country's protracted conflict."
        ),
        (
            "Students protested in Bangladesh demanding transparency after an air force jet "
            "crashed into a school, killing 19. Demonstrators called for accountability and "
            "investigations into military safety protocols, reflecting public anger over "
            "governance failures as the death toll continues to rise."
        ),
    ]
...
def save_test_outputs(
    ...
    with Path(original_leads_file).open("w", encoding="utf-8") as f:
        original_data = {
    ...
    with Path(curated_leads_file).open("w", encoding="utf-8") as f:
        curated_data = {
            "timestamp": test_start_time.isoformat(),
    ...
    if raw_response:
        ...
        with Path(raw_response_file).open("w", encoding="utf-8") as f:
...
def parse_and_display_scores(raw_response: str, leads: list[Lead]) -> list[dict[str, object]]:
    ...
    try:
        ...
        logger.info(
            f"   Score range: {min(r['weighted_score'] for r in detailed_results):.2f} - {max(r['weighted_score'] for r in detailed_results):.2f}"
        )
