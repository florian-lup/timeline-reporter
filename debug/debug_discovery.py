...
def main() -> None:
    ...
    for category_name in DISCOVERY_CATEGORIES[:1]:
        ...
        try:
            ...
            with open(leads_file, 'w', encoding='utf-8') as f:
                ...
                with open(unstructured_file, 'w', encoding='utf-8') as f:
                    json.dump(unstructured_response, f, indent=2, ensure_ascii=False)
    ...
    print(f"Categories tested: 1 (debug mode - first category only)")
    print(f"\n🔄 Testing both STRUCTURED and UNSTRUCTURED outputs:")
    print(f"  📋 Structured: Uses JSON schema to force consistent format")
    print(f"  📝 Unstructured: Natural response format from Perplexity")
    ...
    print(f"\nFile types created:")
    print(f"  - raw_response_*.json: Clean structured JSON response from Perplexity")
    print(f"  - unstructured_response_*.json: Natural chat completion response from Perplexity")
    print(f"  - parsed_leads_*.json: Extracted lead objects from structured response")
    print(f"  - thinking_*.txt: AI reasoning process (<think> content)")
    print(f"  - debug_summary_*.json: Overall test summary")
    print(f"\n💡 Compare structured vs unstructured files to see:")
    print(f"   • How JSON schema affects response format")
    print(f"   • Response length differences") 
    print(f"   • Parsing complexity differences")
