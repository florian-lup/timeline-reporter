...
def generate_audio(
    ...
    for article in articles:
        try:
            ...
            analysis_prompt = TTS_INSTRUCTIONS.format(
                headline=article.headline, summary=article.summary, story=article.story
            )

            logger.info("Generating analysis for: '%s'", article.headline)
            analysis_text = openai_client.chat_completion(
                analysis_prompt, model=CHAT_MODEL
            )
    ...
    logger.info(
        "Generated broadcasts: %d/%d articles processed",
