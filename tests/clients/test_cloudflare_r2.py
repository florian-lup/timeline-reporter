...
def test_init_missing_credentials():
    ...
    with patch("clients.cloudflare_r2.CLOUDFLARE_ACCOUNT_ID", None), \
...
def test_init_with_custom_domain():
    ...
    with patch("clients.cloudflare_r2.boto3.client"), \
...
def test_init_without_custom_domain():
    ...
    with patch("clients.cloudflare_r2.boto3.client"), \
         ...
         patch("clients.cloudflare_r2.R2_CDN_URL_TEMPLATE", "https://{bucket_name}.{account_id}.test-cdn.com"):
...
def test_get_content_type(r2_client):
    ...
    with patch("clients.cloudflare_r2.AUDIO_CONTENT_TYPE_MAP", {"mp3": "audio/mpeg", "ogg": "audio/ogg"}), \
...
def test_upload_audio_success(r2_client, mock_boto3_client):
    ...
    with patch("uuid.uuid4", return_value=test_uuid):
        ...
        # Test the upload
        with patch("clients.cloudflare_r2.STORAGE_PATH_PREFIX", "podcasts"), \
