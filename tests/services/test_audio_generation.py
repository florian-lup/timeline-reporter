...
class TestTTSService:
    ...
    def test_generate_audio_success(self, mock_openai_client, sample_articles):
        ...
        with (
            ...
            patch(
                "services.audio_generation.TTS_INSTRUCTIONS",
                "Analyze: {headline} - {summary} - {story}",
            ),
        ):
            result = generate_audio(sample_articles, openai_client=mock_openai_client)
    ...
    def test_generate_audio_error_handling(self, mock_openai_client, sample_articles):
        ...
        with (
            ...
            result = generate_audio(
                [sample_articles[0]], openai_client=mock_openai_client
            )
    ...
    def test_generate_audio_empty_list(self, mock_openai_client):
        ...
        with patch("services.audio_generation.logger") as mock_logger:
            result = generate_audio([], openai_client=mock_openai_client)
    ...
    def test_generate_audio_prompt_formatting(
        self, mock_openai_client, sample_articles
    ):
        ...
        with (
            ...
            patch(
                "services.audio_generation.TTS_INSTRUCTIONS",
                "Analyze {headline} and {summary} with {story}",
            ),
        ):
            generate_audio([sample_articles[0]], openai_client=mock_openai_client)
    ...
    def test_logging_tts_service(
        self, mock_logger, mock_openai_client, sample_articles
    ):
        ...
        mock_logger.info.assert_any_call("Selected reporter: %s (%s)", "Alex", "alloy")
        mock_logger.info.assert_any_call(
