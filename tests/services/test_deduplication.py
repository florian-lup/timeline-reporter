...
class TestDeduplicationService:
    ...
    def test_deduplicate_events_no_duplicates(self, mock_openai_client, mock_pinecone_client, sample_events, sample_embeddings):
    ...
    def test_deduplicate_events_with_duplicates(self, mock_openai_client, mock_pinecone_client, sample_events, sample_embeddings):
    ...
    def test_deduplicate_events_embedding_content(self, mock_openai_client, mock_pinecone_client, sample_events):
    ...
    def test_deduplicate_events_vector_metadata(self, mock_openai_client, mock_pinecone_client, sample_events):
    ...
    def test_deduplicate_events_multiple_matches(self, mock_openai_client, mock_pinecone_client, sample_events):
        """Test deduplication when event matches multiple existing events."""
        mock_openai_client.embed_text.return_value = [0.1] * 1536
        mock_pinecone_client.similarity_search.side_effect = [
            [("match-1", 0.95), ("match-2", 0.88), ("match-3", 0.82)],  # Multiple matches
            [],  # No duplicates
            []   # No duplicates
        ]
    ...
    @pytest.mark.parametrize("similarity_scores", [
        [("match-1", 0.85)],  # Above threshold
        [("match-1", 0.95), ("match-2", 0.90)],  # Multiple above threshold
        [("match-1", 0.99)],  # Very high similarity
    ])
    def test_deduplicate_events_various_similarity_scores(self, mock_openai_client, mock_pinecone_client, sample_events, similarity_scores):
    ...
    def test_deduplicate_events_search_error_handling(self, mock_openai_client, mock_pinecone_client, sample_events):
