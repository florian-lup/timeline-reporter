...
class TestDiscoveryService:
    ...
    def sample_discovery_response(self):
        """Sample response from OpenAI deep research."""
        return json.dumps([
            {
    ...
    def test_discover_events_success(self, mock_openai_client, sample_discovery_response):
        """Test successful event discovery."""
        mock_openai_client.deep_research.return_value = sample_discovery_response
        ...
        with patch('services.discovery.get_today_formatted', return_value='2024-01-15'):
            with patch('services.discovery.DISCOVERY_INSTRUCTIONS', 'Find events about {topics} on {date}'):
                events = discover_events(mock_openai_client)
        ...
        # Verify OpenAI client was called with correct prompt
        mock_openai_client.deep_research.assert_called_once()
        call_args = mock_openai_client.deep_research.call_args[0][0]
    ...
    def test_discover_events_with_markdown_fences(self, mock_openai_client, sample_malformed_response):
        """Test event discovery with markdown fence-wrapped JSON."""
        mock_openai_client.deep_research.return_value = sample_malformed_response
    ...
    def test_discover_events_empty_response(self, mock_openai_client):
        """Test event discovery with empty JSON array."""
        mock_openai_client.deep_research.return_value = "[]"
    ...
    def test_discover_events_malformed_json(self, mock_openai_client):
        """Test event discovery with malformed JSON."""
        mock_openai_client.deep_research.return_value = "invalid json{"
    ...
    def test_discover_events_missing_fields(self, mock_openai_client):
        ...
        mock_openai_client.deep_research.return_value = response
        
        with patch('services.discovery.get_today_formatted', return_value='2024-01-15'):
            # This should raise KeyError for missing fields
            with pytest.raises(KeyError):
                discover_events(mock_openai_client)
    ...
    def test_discover_events_with_different_topics(self, mock_openai_client, sample_discovery_response, topics_override):
    ...
    def test_logging_events_discovery(self, mock_logger, mock_openai_client, sample_discovery_response):
    ...
    def test_discover_events_with_unicode_content(self, mock_openai_client):
        ...
        mock_openai_client.deep_research.return_value = unicode_response
    ...
    def test_discover_events_prompt_formatting(self, mock_openai_client, sample_discovery_response):
    ...
    def test_parse_events_from_response_edge_cases(self, mock_openai_client):
        ...
        mock_openai_client.deep_research.return_value = complex_response
